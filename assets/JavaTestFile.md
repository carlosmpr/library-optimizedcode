...
- **createChart(XYSeriesCollection dataset)**:
  - Creates an XY line chart titled "BMI Trends" with labels for the x and y-axes.
  - Uses the dataset created in `createDataset()` to populate the chart with data.
  - Configures the plot orientation and rendering options for the chart.
